...
// variables
const isValidUrl =
    /https:\/\/meteo.netitservices.com\/api\/v[0,1]\/devices\/[a-zA-Z0-9]{12}\/poll\?hwtypeId=[0-9]{1,4}/;
...
class Hagelschutz extends utils.Adapter {
    ...
    async getHailData() {
        await axios({
            ...
            .then(response => {
                this.log.debug(
                    `[getHailData]: HTTP status response: ${response.status} ${response.statusText}; config: ${JSON.stringify(response.config)}; headers: ${JSON.stringify(response.headers)}; data: ${JSON.stringify(response.data)}`,
                );
            ...
            .catch(error => {
                if (error.response) {
                    ...
                    this.log.debug(
                        `[getHailData]: HTTP status response: ${error.response.status}; headers: ${JSON.stringify(error.response.headers)}; data: ${JSON.stringify(error.response.data)}`,
                    );
